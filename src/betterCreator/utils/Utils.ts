...
export class Utils {
     ...
     * @return 字符串型颜色值。
     */
    static toHexColor(color: number): string | null {
    ...
    static fromStringColor(value: string): number {
        ...
        if (value.indexOf("rgba(") >= 0 || value.indexOf("rgb(") >= 0) {
            ...
            else return (arr[0] << 16) + (arr[1] << 8) + arr[2];
        } else {
    ...
    static getBaseName(path: string): string {
        ...
        if (i != -1) path = path.substring(i + 1);
        i = path.indexOf("?");
    ...
    static getFileExtension(path: string): string {
        ...
        if (i != -1) {
            ...
            let j = ext.indexOf("?");
            if (j != -1) ext = ext.substring(0, j);
            if (ext === "ls") {
                //lanit.ls ltcb.ls 这类特殊扩展名的支持
                let k = path.lastIndexOf(".", i - 1);
                if (k != -1) {
                    let ext2 = path.substring(k + 1, i + 1) + ext;
                    if (ext2 === "lanit.ls" || ext2 === "ltcb.ls") return ext2;
                }
            }
            return ext;
        } else return "";
    ...
    static replaceFileExtension(path: string, newExt: string, excludeDot?: boolean): string {
        if (!path) return path;
        let i = path.lastIndexOf(".");
        if (newExt.length > 0 && !excludeDot) newExt = "." + newExt;
        if (i != -1) {
            let j = path.indexOf("?", i);
            if (j != -1) return path.substring(0, i) + newExt + path.substring(j);
            else return path.substring(0, i) + newExt;
